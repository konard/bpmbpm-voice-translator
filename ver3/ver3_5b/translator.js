/**
 * Голосовой переводчик ver3.5b — основной JS-модуль (ES-модуль)
 *
 * Исправления по сравнению с ver3.5:
 *  - Исправлена ошибка «The URL '../bergamot/worker/translator-worker.js' is invalid»:
 *    относительный путь к воркеру преобразуется в абсолютный через new URL(..., document.baseURI),
 *    чтобы importScripts внутри Blob-воркера работал корректно.
 *  - Улучшено сообщение об ошибке «Микрофон занят»: теперь даётся более точная диагностика
 *    (может быть аппаратная блокировка, driver-ошибка или политика браузера).
 *
...
const BergamotTranslator = (() => {
  ...
  async function initTranslator(direction, onProgress) {
    ...
    // FIX: Преобразуем относительный путь в абсолютный URL через document.baseURI.
    // Это необходимо, потому что importScripts() внутри Blob-воркера разрешает
    // относительные пути относительно blob:-URL, а не страницы — что всегда
    // приводит к ошибке «The URL '...' is invalid».
...
const App = (() => {
  ...
  async function init() {
    ...
    Logger.log("=== Голосовой переводчик ver3.5b запускается ===", "info");
  ...
  async function checkMicrophone() {
    ...
    try {
      ...
      // FIX: Улучшена диагностика ошибки NotReadableError.
      // Эта ошибка возникает не только когда микрофон занят другим приложением,
      // но и при аппаратных сбоях, проблемах с драйвером или политике браузера.
      // Прежнее сообщение «Закройте другие приложения» вводило в заблуждение.
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        ...
        // Причина: аппаратная ошибка, проблема с драйвером или временная блокировка ОС.
        // Не всегда означает, что микрофон занят другим приложением.
        Logger.log(
          `Не удалось получить доступ к микрофону (${err.name}: ${err.message}). ` +
          `Возможные причины: микрофон занят другим приложением (видеозвонки, Zoom, Teams, Discord, OBS), ` +
          `проблема с драйвером звуковой карты, или аппаратная ошибка устройства. ` +
          `Попробуйте: 1) закрыть другие приложения, использующие микрофон; ` +
          `2) отключить и подключить микрофон снова; ` +
          `3) перезапустить браузер; ` +
          `4) проверить настройки звука в системе.`,
          "warn"
        );
        InitChecklist.setItem("chk-mic", "warn", "Микрофон недоступен — см. журнал для диагностики");
      } else {
